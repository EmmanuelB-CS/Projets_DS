...
class DfAnalysis:
    ...
    def duplicate_check(self):
        ...
        if duplicated_rows != 0:
            print(f'There are {duplicated_rows} rows that are duplicated so we need to drop them')
    ...
    def identify_outliers(self, method='zscore', threshold=3):
        ...
        Args:
            ...
            threshold (int or float): The threshold for identifying outliers.
        ...
        outlier_percentages = outliers.mean(axis=0) * 100
        outlier_percentages_df = outlier_percentages.to_frame(name='Percentage of Outliers')
        outlier_percentages_df.index.name = 'Column'

        # Finding the index of outliers for at least one feature
        outlier_indices = outliers.any(axis=1)
        outlier_list = outlier_indices[outlier_indices].index.tolist()

        return outlier_list, outlier_percentages_df

    def find_correlated_variables(self, threshold=0.8):
        """
    ...
    def plot_pairplot(self, target=None):
        ...
        plt.savefig(f'img/pairplot{"_"+target if target else ""}.png')
        plt.close()

    def plot_categorical_distribution(self, n_cols=3):
        """
        ...
        plt.tight_layout()
        plt.savefig('img/categorical_distribution.png')
        plt.close()
    ...
    def handle_feature_selection(self, target_column, k = None):
        ...
        For non-linear dependencies, consider using methods that can model complex interactions, 
        such as mutual information or model-based feature selection techniques.
        ...
        number of observations is not significantly larger than the number of features. For high-dimensional 
        datasets, it is recommended to use more sophisticated feature selection methods that can handle 
        the complexity and interactions among features.
        ...
        the original dataset. Consider adjusting the value of k or using alternative feature selection 
...
class DataPreprocessing:
    def __init__(self, df, columns_to_exclude=[], categorical_features=[]):
        """
    ...
    def handle_imbalanced_data(self, target_column, method='oversampling'):
        """
        ...
        if method == 'oversampling':
            ...
            X = pd.get_dummies(self.df.drop(target_column, axis=1), drop_first=True)
        ...
        resampled_df = pd.concat([pd.DataFrame(X_resampled, columns=X.columns), pd.DataFrame(y_resampled, columns=[target_column])], axis=1)
        
        # Optionally, update the class attribute if you want to keep the resampled data for further use
